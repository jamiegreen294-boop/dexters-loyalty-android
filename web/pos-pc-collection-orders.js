...
const PRINTED_RECEIPTS='dexters_pc_loyalty_order_receipts_v2';
...
function confirmInPos(title,message,confirmLabel='CONFIRM'){return new Promise(resolve=>{const d=modalC(title,'<p style="font-size:18px">'+escC(message)+'</p><div class="actions"><button class="cancel collectionConfirmCancel">CANCEL</button><button class="confirm collectionConfirmOk">'+escC(confirmLabel)+'</button></div>');let done=false;const finish=value=>{if(done)return;done=true;d.remove();resolve(value)};d.querySelector('.collectionConfirmCancel').onclick=()=>finish(false);d.querySelector('.collectionConfirmOk').onclick=()=>finish(true)})}
function waiting(o){return ['pending','amended'].includes(String(o.status))}
function asList(v){if(Array.isArray(v))return v;if(v==null||v==='')return[];if(typeof v==='string'){try{const parsed=JSON.parse(v);return Array.isArray(parsed)?parsed:[v]}catch{return[v]}}return[v]}
function paymentState(o){const raw=o.payment_status??o.payment_state??o.payment_method??(o.paid===true?'paid':o.paid===false?'unpaid':'');const value=String(raw||'unpaid').trim().toLowerCase();return {value,label:value==='paid'||value==='complete'||value==='completed'||value==='succeeded'?'PAID':value==='part_paid'||value==='part-paid'||value==='partial'?'PART PAID':value==='cash_on_collection'||value==='pay_on_collection'?'PAY ON COLLECTION':value.replaceAll('_',' ').toUpperCase()||'UNPAID'}}
function mapItem(i){const mods=asList(i?.modifiers??i?.modifier_options??i?.options).flatMap(v=>{if(v==null)return[];if(typeof v==='string')return[v];if(Array.isArray(v?.options))return v.options.map(x=>typeof x==='string'?x:(x?.name||x?.label||'')).filter(Boolean).map(x=>(v.name||v.group_name)?(v.name||v.group_name)+': '+x:x);return[v.name||v.label||v.option_name||'']}).filter(Boolean);for(const v of asList(i?.removed??i?.removed_ingredients))if(v)mods.push('REMOVE: '+String(v));return {id:i?.id||i?.item_id||'',name:String(i?.base_name||i?.name||i?.item_name||'Item'),qty:Math.max(1,Number(i?.qty??i?.quantity)||1),price:i?.price??i?.unit_price??i?.unit??'',modifiers:mods}}
function mapOrder(o){const p=paymentState(o||{});return {...o,id:String(o?.id||''),order_number:o?.order_number??o?.number??'',customer_name:o?.customer_name||o?.customer?.name||o?.name||'Customer',customer_phone:o?.customer_phone||o?.customer?.phone||o?.phone||'',items:asList(o?.items??o?.order_items).map(mapItem),order_notes:o?.order_notes??o?.notes??'',collection_time:o?.collection_time??o?.requested_for??o?.collection_slot??'ASAP',payment_status:p.value,payment_label:p.label,status:String(o?.status||'pending').toLowerCase()}}
function activeOrder(o){return ['pending','amendment_required','amended','accepted','preparing','ready'].includes(String(o?.status||'').toLowerCase())}
...
async function toggleCollection(){if(toggleBusy)return;const next=!collectionOpen;if(!await confirmInPos((next?'Open':'Close')+' collection ordering',(next?'Open':'Close')+' app collection ordering on the PC TEST system?',next?'OPEN ORDERS':'CLOSE ORDERS'))return;toggleBusy=true;const b=$c('pcCollectionToggleBtn');if(b)b.textContent='SAVING…';try{const d=await api({action:'set_enabled',enabled:next});collectionOpen=!!d.enabled;paintToggle()}catch(e){alert(e.message);paintToggle()}finally{toggleBusy=false}}
...
async function poll(force=false){if(busy||!S?.session?.access_token)return;busy=true;try{const d=await api({action:'kds_orders'});orders=(d.orders||[]).map(mapOrder).filter(activeOrder);reasons=d.rejection_reasons||[];paint();
  // Print every active app order once, even if another device accepts it very quickly.
  const unprinted=orders.filter(o=>o?.id&&!printedReceiptIds().has(String(o.id))).sort((a,b)=>String(a.created_at||'').localeCompare(String(b.created_at||'')));
  for(const o of unprinted){try{await printLoyaltyOrderReceipt(o)}catch(e){const b=$c('pcCollectionOrdersBtn');if(b)b.title='Receipt print: '+String(e?.message||e);break}}
  const key=orders.filter(waiting).map(o=>o.id+':'+o.status).join(',');if(key&&key!==lastWaiting){lastWaiting=key;if(force!==true){try{navigator.vibrate?.([180,100,180])}catch{};const newest=orders.filter(waiting).sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||''))).find(o=>!alertedKeys().has(alertKey(o)));if(newest)setTimeout(()=>showLiveOrder(newest),100)}}if(!key)lastWaiting=''}catch(e){const b=$c('pcCollectionOrdersBtn');if(b)b.title='Collection feed: '+e.message}finally{busy=false}}
...
async function subscribeRealtime(){if(realtimeChannel||!S?.session?.access_token)return;try{await loadRealtimeLibrary();realtimeClient=window.supabase.createClient(U,K,{auth:{persistSession:false,autoRefreshToken:false},global:{headers:{Authorization:'Bearer '+S.session.access_token}}});await realtimeClient.realtime.setAuth(S.session.access_token);realtimeChannel=realtimeClient.channel('pc-pos-collection-orders-v3').on('postgres_changes',{event:'*',schema:'public',table:'collection_orders'},payload=>{const row=payload.new&&Object.keys(payload.new).length?mapOrder(payload.new):null;if(row&&activeOrder(row)){const at=orders.findIndex(x=>x.id===row.id);if(at<0)orders.unshift(row);else orders[at]={...orders[at],...row};paint();if(['INSERT','UPDATE'].includes(String(payload.eventType||'')))printLoyaltyOrderReceipt(row).catch(()=>{});if(waiting(row)&&['INSERT','UPDATE'].includes(String(payload.eventType||'')))setTimeout(()=>showLiveOrder(row),80)}else if(row?.id){orders=orders.filter(x=>x.id!==row.id);paint()}poll()}).subscribe(status=>{const b=$c('pcCollectionOrdersBtn');if(b)b.dataset.realtime=status==='SUBSCRIBED'?'connected':'connecting';if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'){realtimeChannel=null;clearTimeout(realtimeRetry);realtimeRetry=setTimeout(subscribeRealtime,3000)}})}catch(e){const b=$c('pcCollectionOrdersBtn');if(b)b.title='Realtime reconnecting: '+e.message;realtimeChannel=null;clearTimeout(realtimeRetry);realtimeRetry=setTimeout(subscribeRealtime,3000)}}
function install(){if($c('pcCollectionOrdersBtn'))return;const top=document.querySelector('.top')||document.body,b=document.createElement('button'),t=document.createElement('button');b.id='pcCollectionOrdersBtn';b.textContent='Collection Orders';top.insertBefore(b,$c('staffBtn')||null);b.onclick=openInbox;t.id='pcCollectionToggleBtn';t.textContent='COLLECTION…';top.insertBefore(t,b);t.onclick=toggleCollection;const s=document.createElement('style');s.textContent='.pcCollectionCount{display:inline-grid;place-items:center;min-width:24px;height:24px;padding:0 6px;border-radius:999px;background:#dc3345;color:#fff}.pcCollectionOn{background:#15803d!important;color:#fff!important}.pcCollectionOff{background:#991b1b!important;color:#fff!important}.pcCollectionFlash{background:#ffd43b!important;color:#08101d!important;animation:pcCollectionPulse 1s step-end infinite}@keyframes pcCollectionPulse{50%{background:#dc3345;color:#fff;box-shadow:0 0 0 5px #dc334555}}@media(prefers-reduced-motion:reduce){.pcCollectionFlash{animation:none;box-shadow:0 0 0 4px #dc3345}}';document.head.appendChild(s);poll();refreshToggle();subscribeRealtime();setInterval(poll,60000);setInterval(refreshToggle,60000)}
