...
function style(){
 ...
 '.nav{display:none!important}#dextersRecoveryTop{display:flex;gap:10px;align-items:center;justify-content:space-between;margin:8px 2px 14px;padding:0 2px}#dextersRecoveryTop button{width:auto;padding:7px 0;border:0;background:transparent;color:#ffd095;font-weight:700;font-size:18px;letter-spacing:.01em;text-shadow:0 2px 8px #000}#dextersRecoveryTop .admin{margin-left:auto;font-size:14px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}#dextersRecoveryBottom{position:fixed;left:0;right:0;bottom:0;z-index:40;background:rgba(8,16,29,.97);backdrop-filter:blur(12px);border-top:1px solid #243651;padding:7px max(8px,env(safe-area-inset-right)) calc(7px + env(safe-area-inset-bottom)) max(8px,env(safe-area-inset-left))}#dextersRecoveryBottom .inner{width:min(520px,100%);margin:auto;display:grid;grid-template-columns:repeat(5,1fr);gap:3px}#dextersRecoveryBottom button{border:0;background:transparent;color:#9ba8ba;padding:7px 1px;border-radius:12px;font-weight:800;font-size:19px}#dextersRecoveryBottom button span{display:block;font-size:10px;margin-top:2px}#dextersRecoveryBottom button.active{background:#21314c;color:var(--yellow)}#dextersLiveOrdersCard .order-row{background:#0d1829;border:1px solid #324866;border-radius:14px;padding:12px;margin:9px 0}#dextersLiveOrdersCard .order-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin-top:10px}#dextersLiveOrdersCard .order-step{font-size:10px;text-align:center;padding:7px 3px;border-radius:9px;background:#18263b;color:#8fa0b6;font-weight:800}#dextersLiveOrdersCard .order-step.on{background:#244c37;color:#b9f3ce}#dextersLiveOrdersCard .order-step.current{outline:2px solid var(--yellow)}';
...
function ensureBottom(){
 ...
 n.querySelectorAll('button[data-go]').forEach(b=>b.onclick=()=>{go(b.dataset.go);n.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b))});
 const live=n.querySelector('#dextersNavLiveOrder');if(live)live.onclick=()=>{go('homePage');setTimeout(()=>{document.getElementById('dextersLiveOrdersCard')?.scrollIntoView({behavior:'smooth',block:'start'})},80);n.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===live))};
}
async function role(){
