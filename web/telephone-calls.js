(() => {
  ...
  const sb=window.supabase?.createClient(U,K,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false,storage:window.localStorage,storageKey:'dexters-backoffice-auth-v1'}});
  let events=[];
  ...
  async function api(body){const {data}=await sb.auth.getSession(),token=data.session?.access_token;if(!token)throw Error('Your Back Office session has expired. Sign in again.');const r=await fetch(U+'/functions/v1/telephone-events-admin-test',{method:'POST',headers:{apikey:K,Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Telephone history could not be loaded.');return d}
