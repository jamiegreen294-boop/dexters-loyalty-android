...
async function refresh(){
 ...
 const l=await call({action:'list',collection_date:cfg.collection_date});const allOrders=l.orders||[];orders=allOrders.filter(o=>{const ps=String(o.payment_status||'').toLowerCase(),st=String(o.status||'').toLowerCase(),total=Number(o.total_pence||0),bal=Number(o.balance_pence||0);return ps==='completed'||ps==='paid_full'||ps==='deposit_paid'||(total>0&&bal<total)||['pending','accepted','preparing','ready','collected'].includes(st)});
