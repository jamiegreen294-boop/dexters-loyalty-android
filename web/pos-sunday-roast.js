...
function orderCard(o){const closed=['collected','rejected','no_show','payment_failed'].includes(String(o.status||''));return '<article class="srOrder"><div><b>SR-'+String(o.order_number||'').padStart(3,'0')+' · '+esc(o.customer_name||'Customer')+'</b><br>'+esc(o.collection_date)+' at '+esc(o.collection_slot)+'<br>'+(o.items||[]).map(i=>Number(i.qty||0)+' × '+esc(i.name)).join(' · ')+(o.order_notes?'<div class="srOrderNote"><b>NOTE:</b> '+esc(o.order_notes)+'</div>':'')+'<br><small>'+esc(String(o.status||'pending').toUpperCase())+' · '+mp(o.total_pence)+' · '+(Number(o.balance_pence||0)>0?'DUE '+mp(o.balance_pence):'PAID')+'</small></div><div class="srOrderBtns">'+(!closed?'<button data-sr-edit="'+o.id+'">EDIT</button>':'')+(Number(o.balance_pence||0)>0&&!closed?'<button data-sr-pay="'+o.id+'">TAKE PAYMENT</button>':'')+(String(o.status)==='pending'?'<button data-sr-ready="'+o.id+'">READY</button>':'')+(String(o.status)==='ready'?'<button data-sr-collected="'+o.id+'">COLLECTED</button>':'')+'</div></article>'}
...
function beginEdit(o){if(!o)return;editing=o;selectedCustomer={id:o.customer_id,full_name:o.customer_name,phone:o.customer_phone,loyalty_code:o.loyalty_code||''};state=stateFromOrder(o);$('srFormTitle').textContent='Edit SR-'+String(o.order_number).padStart(3,'0');$('srSlot').value=o.collection_slot||'';$('srOrderNotes').value=o.order_notes||'';$('srPaymentRow').style.display='none';$('srCustomerSearchWrap').style.display='none';renderCustomer();renderEditor();$('srSave').textContent='SAVE ORDER CHANGES';$('srForm').scrollIntoView({behavior:'smooth',block:'start'})}
function resetNew(){editing=null;selectedCustomer=null;walkIn=false;state=blank();$('srFormTitle').textContent='Add Sunday Roast in store';$('srPaymentRow').style.display='grid';$('srCustomerSearchWrap').style.display='block';$('srSave').textContent='CREATE LIVE SUNDAY ROAST ORDER';$('srPaid').value='0.00';if($('srOrderNotes'))$('srOrderNotes').value='';renderCustomer();renderEditor()}
...
function install(){
 if(!STANDALONE){
   const launch=()=>{const w=window.open('./pos-sunday-roast-tab.html','dextersSundayRoast');if(!w)location.href='./pos-sunday-roast-tab.html'};
   let b=$('pcSundayBtn');if(!b){b=document.createElement('button');b.id='pcSundayBtn';b.textContent='Sunday Roast';document.querySelector('.top')?.insertBefore(b,$('staffBtn')||null)}b.onclick=e=>{e.preventDefault();launch()};
   window.DextersSundayOrders={open:launch};window.DextersSundayCreate=launch;window.DextersSundayEdit=launch;return;
 }
 ...
 if(STANDALONE)setTimeout(open,0);
}
