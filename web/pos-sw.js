const CACHE='dexters-pos-shell-v2';
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname==='/pos.html'||u.pathname==='/pos-test.html'){
    e.respondWith(fetch(new Request(e.request,{cache:'reload'})).then(r=>{const x=r.clone();caches.open(CACHE).then(c=>c.put('/pos.html',x));return r}).catch(()=>caches.match('/pos.html')));
  }
});
