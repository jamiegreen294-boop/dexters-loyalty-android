(()=>{
  ...
  function pair(){const current=localStorage.getItem(PAIR_KEY)||'';if(typeof modal==='function'){
    const d=modal('Pair Barcode Scanner','<p>Enter the 6-digit code shown by the Dexter scanner bridge on this Windows PC.</p><input id="scannerPairCode" class="field" inputmode="numeric" maxlength="6" autocomplete="one-time-code" value="'+current.replace(/[^0-9]/g,'')+'" placeholder="000000"><div class="actions"><button id="scannerPairSave" class="confirm">PAIR SCANNER</button></div><p style="color:#9eb0c5;font-size:13px">Keyboard-wedge scanners still work even without the bridge.</p>',false);
