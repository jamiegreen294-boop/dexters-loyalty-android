...
function cleanupLegacy(){const card=$('mySpinPrizeCard');if(!card)return;const copy='Claim and redeem this prize through the Dexter’s app when placing a collection order. It cannot be redeemed in store or by staff on a terminal.';const p=card.querySelector('p.muted');if(p&&p.textContent!==copy)p.textContent=copy;card.querySelectorAll('#redeemSpinBtn,[data-redeem-spin]').forEach(b=>{if(!b.hidden)b.hidden=true;if(!b.disabled)b.disabled=true;if(b.getAttribute('aria-hidden')!=='true')b.setAttribute('aria-hidden','true')})}
