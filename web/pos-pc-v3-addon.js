...
async function hardwarePrintSale(sale,opts={}){
  try{
    ...
    // Receipt printing and drawer kicking are deliberately separate. The drawer
    // uses the exact DextersHardware.bridge path proven by Hardware > TEST DRAWER.
    return launchHardwarePrint(txt,{loyalty:!!customer,claim,openDrawer:!!opts.openDrawer});
...
async function kickCashDrawer(){
  try{
    // Give the receipt protocol handoff time to finish, then use the exact
    // working Hardware > TEST DRAWER code path.
    await new Promise(r=>setTimeout(r,650));
    if(window.DextersHardware?.bridge){
      return window.DextersHardware.bridge("",{drawer:true});
    }
    // Fallback only if the hardware module has not loaded for some reason.
    const pulse='\x1b\x70\x00\x3c\x78\x1b\x70\x01\x3c\x78';
    return launchHardwarePrint(pulse,{openDrawer:false});
  }catch(e){
...
function cardConfirm(title,total,onConfirm){ensurePayCss();const d=document.createElement('div');d.className='modal';d.style.zIndex='20050';d.innerHTML='<div class="box pcPayBox"><h2>'+title+' · CARD</h2><div class="pcPaySummary"><div style="color:#9eb0c5">CARD AMOUNT</div><div class="pcPayTotal">'+cashMoney(total)+'</div><p style="color:#9eb0c5">PC TEST: this records an approved card payment without charging a real card.</p><div class="pcPayActions"><button class="pcPayCancel">CANCEL</button><button class="pcPayConfirm">APPROVE TEST PAYMENT</button></div></div></div>';document.body.appendChild(d);d.querySelector('.pcPayCancel').onclick=()=>d.remove();d.querySelector('.pcPayConfirm').onclick=()=>{d.remove();onConfirm()};return d}
async function squareCardConfirm(title,total,onConfirm){ensurePayCss();const d=document.createElement('div');d.className='modal';d.style.zIndex='20050';d.innerHTML='<div class="box pcPayBox"><h2>'+title+' · SQUARE</h2><div class="pcPaySummary"><div style="color:#9eb0c5">AMOUNT</div><div class="pcPayTotal">'+cashMoney(total)+'</div><p class="pcSquareWait" style="color:#9eb0c5">Sending payment to Foodhub terminal…</p><p><small>Square will open automatically on the Foodhub terminal. The sale completes only after Square approves it.</small></p><div class="pcPayActions"><button class="pcSquareCancel pcPayCancel">CANCEL / BACK</button></div></div></div>';document.body.appendChild(d);const state=d.querySelector('.pcSquareWait'),cancel=d.querySelector('.pcSquareCancel');cancel.onclick=async()=>{cancel.disabled=true;state.textContent='Cancelling Square request…';try{if(typeof window.DextersPcSquareCancel==='function')await window.DextersPcSquareCancel()}catch{}if(document.body.contains(d))d.remove()};try{if(typeof window.DextersPcSquarePay!=='function')throw Error('Square bridge is not loaded');state.textContent='Waiting for Foodhub / Square approval…';const result=await window.DextersPcSquarePay(total,title);if(!document.body.contains(d))return d;d.remove();await onConfirm(result)}catch(e){if(document.body.contains(d)){state.textContent='Payment not completed: '+String(e?.message||e);cancel.disabled=false;cancel.textContent='BACK TO ORDER'}}return d}
window.DextersPcTakePayment=(amount,title='Payment')=>new Promise((resolve,reject)=>{
 const total=Math.max(0,Number(amount||0));
 if(total<=0){reject(new Error('No payment amount due.'));return}
 paymentMethodModal(title,total,
   (tender,change)=>resolve({method:'cash',tendered:Number(tender||0),change:Number(change||0)}),
   result=>resolve({method:'square',transaction_id:result?.transaction_id||'',square_result:result||null}),
   true
 );
});
function cashPay(){const total=cartTotal();if(total<=0){modal('Payment','<p>Add items to the basket first.</p>');return}cashKeypad('Sale',total,async(tender,change)=>{const sale=recordSale('cash',tender,change);try{const sent=await sendPaidOrderToLiveKds('cash',tender);await hardwarePrintSale(sale,{openDrawer:true});clearAfterPaidSend();const d=modal('Payment complete','<div class="pcPayTotal">CHANGE '+cashMoney(change)+'</div><p>Sent to live KDS'+(sent?.kds_order_number?' · KDS #'+sent.kds_order_number:'')+'. Receipt sent to POS-80.</p>');setTimeout(()=>{if(document.body.contains(d))d.remove()},3500)}catch(e){modal('KDS send failed','<p>'+String(e.message||e)+'</p><p>The basket has been kept so you can retry. The receipt was not printed.</p>')}})}
function cardPay(){const total=cartTotal();if(total<=0){modal('Payment','<p>Add items to the basket first.</p>');return}squareCardConfirm('Sale',total,async(result)=>{const sale=recordSale('card',total,0,'sale',{square_transaction_id:result?.transaction_id||''});try{const sent=await sendPaidOrderToLiveKds('card',total);clearAfterPaidSend();await hardwarePrintSale(sale);const d=modal('Payment complete','<div class="pcPayTotal">'+cashMoney(total)+'</div><p>Square approved on Foodhub terminal. Sent to live KDS'+(sent?.kds_order_number?' · KDS #'+sent.kds_order_number:'')+'. Receipt sent to POS-80.</p>');setTimeout(()=>{if(document.body.contains(d))d.remove()},2500)}catch(e){modal('KDS send failed','<p>'+String(e.message||e)+'</p><p>The payment was approved but the basket has been kept because KDS/printing failed.</p>')}})}
function choosePay(){const total=cartTotal();if(total<=0){modal('Payment','<p>Add items to the basket first.</p>');return}paymentMethodModal('Pay',total,async(tender,change)=>{const sale=recordSale('cash',tender,change);try{const sent=await sendPaidOrderToLiveKds('cash',tender);await hardwarePrintSale(sale,{openDrawer:true});clearAfterPaidSend();const d=modal('Payment complete','<div class="pcPayTotal">CHANGE '+cashMoney(change)+'</div><p>Sent to live KDS'+(sent?.kds_order_number?' · KDS #'+sent.kds_order_number:'')+'. Receipt sent to POS-80 and cash drawer should open.</p>');setTimeout(()=>{if(document.body.contains(d))d.remove()},3500)}catch(e){modal('KDS send failed','<p>'+String(e.message||e)+'</p><p>The basket has been kept so you can retry. The receipt was not printed.</p>')}},async(result)=>{const sale=recordSale('card',total,0,'sale',{square_transaction_id:result?.transaction_id||''});try{const sent=await sendPaidOrderToLiveKds('card',total);clearAfterPaidSend();await hardwarePrintSale(sale);const d=modal('Payment complete','<div class="pcPayTotal">'+cashMoney(total)+'</div><p>Square approved on Foodhub terminal. Sent to live KDS'+(sent?.kds_order_number?' · KDS #'+sent.kds_order_number:'')+'. Receipt sent to POS-80.</p>');setTimeout(()=>{if(document.body.contains(d))d.remove()},2500)}catch(e){modal('KDS send failed','<p>'+String(e.message||e)+'</p><p>The payment was approved but the basket has been kept because KDS/printing failed.</p>')}},true)}
...
async function applyLiveCreditPayment(acct,name,balance,amount,method,tender=amount,change=0,squareResult=null){
  const pence=Math.round(Number(amount||0)*100);
  if(!pence)throw Error('Enter a payment amount.');
  const active=await currentStaffSession();
  const r=await fetch(U+'/rest/v1/rpc/credit_record_payment',{method:'POST',headers:{apikey:K,Authorization:'Bearer '+active.access_token,'Content-Type':'application/json'},body:JSON.stringify({p_account_id:acct,p_amount_pence:pence,p_description:'Payment received in store',p_source:'pos',p_metadata:{payment_method:method,square_transaction_id:squareResult?.transaction_id||null}})});
  const x=await r.json().catch(()=>null);
  if(!r.ok)throw Error(x?.message||x?.error||'Could not update Money Owed balance.');
  const row={id:'CREDIT-'+Date.now(),created_at:new Date().toISOString(),kind:'money_owed',method,amount,total:amount,tendered:tender,change,account_id:acct,customer_name:name,balance_before_pence:balance,balance_after_pence:Math.max(0,balance-pence),live_payment_id:x,mode:'LIVE'};
  const rows=readSales();rows.unshift(row);saveSales(rows);
  const receipt={...row,items:[{name:'Money Owed payment · '+name,qty:1,unit:Number(amount),mods:[]}]};
  await launchHardwarePrint(buildStandardReceipt(receipt),{openDrawer:method==='cash'});
  const d=modal('Money Owed payment recorded','<div class="pcPayTotal">'+cashMoney(amount)+'</div><p><b>'+name+'</b></p><p>Remaining balance: <b>'+cashMoney(row.balance_after_pence/100)+'</b></p>'+(change?'<div class="pcChange ok">Change due: '+cashMoney(change)+'</div>':'')+'<p class="ok"><b>LIVE BALANCE UPDATED ✓</b></p>');
  setTimeout(()=>{if(document.body.contains(d))d.remove()},3500);
  return row;
}
function creditPayLive(acct,name,balance){const max=balance/100;ensurePayCss();const d=document.createElement('div');d.className='modal';d.innerHTML='<div class="box pcPayBox"><h2>Money Owed · '+name+'</h2><div class="pcPaySummary"><div style="color:#9eb0c5">CURRENT BALANCE</div><div class="pcPayTotal">'+cashMoney(max)+'</div><label>Amount customer is paying</label><input id="pcDebtAmount" class="field" inputmode="decimal" placeholder="0.00" value=""><div class="pcQuick"><button data-debt="full">FULL BALANCE</button><button data-debt="5">£5</button><button data-debt="10">£10</button><button data-debt="20">£20</button><button data-debt="clear">CLEAR</button></div><div class="pcPayActions"><button class="pcPayCancel">CANCEL</button><button class="pcPayConfirm">CONTINUE</button></div></div></div>';document.body.appendChild(d);const inp=d.querySelector('#pcDebtAmount');d.querySelectorAll('[data-debt]').forEach(b=>b.onclick=()=>{const v=b.dataset.debt;inp.value=v==='full'?max.toFixed(2):v==='clear'?'':Number(v).toFixed(2);inp.focus()});d.querySelector('.pcPayCancel').onclick=()=>d.remove();d.querySelector('.pcPayConfirm').onclick=()=>{const amount=Number(String(inp.value||'').replace(',','.'));if(!amount||amount<=0||amount>max){inp.focus();return}d.remove();paymentMethodModal('Money Owed · '+name,amount,async(tender,change)=>{try{await applyLiveCreditPayment(acct,name,balance,amount,'cash',tender,change)}catch(e){modal('Payment failed','<p>'+String(e.message||e)+'</p>')}},async(result)=>{try{await applyLiveCreditPayment(acct,name,balance,amount,'card',amount,0,result)}catch(e){modal('Payment failed','<p>'+String(e.message||e)+'</p>')}},true)}}
function moneyOwed(){const d=modal('Money Owed · Pay in Store','<input id="pcCreditQ" class="field" placeholder="Customer name, phone or email"><button id="pcCreditSearch" class="confirm" style="border:0;border-radius:10px;padding:10px 14px;font-weight:900">Search</button><div id="pcCreditResults" style="margin-top:10px"></div><p style="color:#9eb0c5">Payments taken here update the live Money Owed balance.</p>',true);d.querySelector('#pcCreditSearch').onclick=async()=>{const out=d.querySelector('#pcCreditResults');out.textContent='Searching…';try{const rows=await creditSearch(d.querySelector('#pcCreditQ').value);out.innerHTML=rows.length?rows.map((x,i)=>{const name=x.customer_name||x.full_name||x.name||'Customer';const bp=Number(x.balance_pence||x.balance||0);return '<div class="held"><b>'+name+'</b><br>Balance: <b>'+cashMoney(bp/100)+'</b><br><button class="pcCreditPay" data-i="'+i+'">TAKE PAYMENT</button></div>'}).join(''):'No customers found.';out.querySelectorAll('.pcCreditPay').forEach(b=>b.onclick=()=>{const x=rows[Number(b.dataset.i)];creditPayLive(x.id||x.account_id,x.customer_name||x.full_name||x.name||'Customer',Number(x.balance_pence||x.balance||0))})}catch(e){out.textContent=e.message}}}
