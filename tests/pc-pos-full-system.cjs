...
async function installMocks(context){
 ...
 await context.route('https://bpnkouymdvcogeaqjmxl.supabase.co/**',async r=>{
  ...
  else if(p.includes('sunday-roast-pc-pos-test-api'))x=a==='outstanding'?{orders:[]}:a==='list'?{orders:[ROAST]}:a==='customer_search'?{customers:[CUSTOMER]}:{ok:true,order:{...ROAST,...b,id:b.id||'r2'},orders:[]};
...
(async()=>{
 ...
 await closeAll(page);await clickId(page,'pcSundayBtn');await page.waitForSelector('#srNewWalkIn');await page.locator('#srNewWalkIn').click();await page.waitForSelector('#srCustomerQuery');await page.locator('#srCustomerQuery').fill('Browser');await page.locator('#srCustomerGo').click();await page.waitForSelector('.srPickCustomer');await page.locator('.srPickCustomer').first().click();await page.waitForSelector('#soName');const roastText=await page.locator('.modal:not(.hide)').last().innerText();for(const t of ['Roast Chicken Dinner','Roast Beef Dinner','Adult Mixed Roast Dinner','Kids’ Roast Chicken Dinner','Kids’ Roast Beef Dinner','Kids Mixed Roast Dinner','Extra Yorkshire Pudding','Extra Gravy','Extra Stuffing','Extra Roast Potatoes','Extra Chicken','Extra Beef'])if(!roastText.includes(t))fail('Sunday Roast option missing '+t);await page.locator('.srPlus[data-p="meals"][data-k="mixed"]').click();await page.locator('.srPlus[data-p="extras"][data-k="gravy"]').click();if((await page.locator('#soTotal').textContent())!=='£15.99')fail('Sunday Roast total wrong',await page.locator('#soTotal').textContent());console.log('PASS SUNDAY ROAST FULL OPTIONS + EXTRAS');
