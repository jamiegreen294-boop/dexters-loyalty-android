...
async function installMocks(context){
 ...
 await context.route('https://bpnkouymdvcogeaqjmxl.supabase.co/**',async r=>{
  ...
  else if(p.includes('pc-pos-square-bridge')){if(a==='create')x={ok:true,request:{id:'sq-full',amount_pence:Number(b.amount_pence||0),status:'pending'}};else if(a==='status')x={ok:true,request:{id:'sq-full',status:'approved',transaction_id:'square-full-system-test'}};else if(a==='cancel')x={ok:true,request:{id:'sq-full',status:'cancelled'}};else x={ok:true,request:null};}
...
(async()=>{
 ...
 const imageState=await page.evaluate(()=>{const cards=[...document.querySelectorAll('.pcCatCard')],wraps=[...document.querySelectorAll('.pcCatImage')],imgs=[...document.querySelectorAll('.pcCatImage img')];const bad=imgs.map((im,i)=>{const r=im.getBoundingClientRect(),fit=getComputedStyle(im).objectFit;return{index:i,w:r.width,h:r.height,fit,src:im.src}}).filter(x=>x.w<=0||x.h<=0||x.fit!=='cover'||!x.src.startsWith('https://'));return{cards:cards.length,wraps:wraps.length,imgs:imgs.length,bad}});if(imageState.cards<8||imageState.wraps!==imageState.cards||imageState.imgs!==imageState.cards||imageState.bad.length)fail('Category photos failed',imageState);console.log('PASS CATEGORY PHOTOS',imageState.cards);
 ...
 await page.evaluate(()=>window.pcShowCategories?.());await page.locator('.pcCatCard').filter({hasText:'Hot Rolls'}).click();await page.locator('.item[data-item="i1"]').click();await page.waitForSelector('#modifierModal:not(.hide)');await page.locator('#modifierModal [data-o="brown"]').click();await page.locator('#modAdd').click();await clickId(page,'payBtn');await page.waitForSelector('.cardM');await page.evaluate(()=>document.querySelector('.cardM')?.click());await page.waitForFunction(()=>JSON.parse(localStorage.getItem('dexters_pc_pos_sales_v3')||'[]').some(x=>x.method==='card'&&x.square_transaction_id==='square-full-system-test'),null,{timeout:7000});sales=await page.evaluate(()=>JSON.parse(localStorage.getItem('dexters_pc_pos_sales_v3')||'[]'));if(!sales.some(x=>x.method==='card'&&x.square_transaction_id==='square-full-system-test'))fail('Square card payment not recorded',sales.slice(0,3));console.log('PASS REQUIRED MODIFIER + SQUARE PAYMENT');
 ...
 await closeAll(page);await clickId(page,'pcMoneyBtn');await page.waitForSelector('#pcCreditQ');await page.locator('#pcCreditQ').fill('Browser');await page.locator('#pcCreditSearch').click();await page.waitForSelector('.pcCreditPay');await page.locator('.pcCreditPay').click();await page.locator('#pcDebtAmount').fill('5.00');await page.evaluate(()=>document.querySelector('.pcPayConfirm')?.click());await page.waitForSelector('.cardM');await page.evaluate(()=>document.querySelector('.cardM')?.click());await page.waitForFunction(()=>JSON.parse(localStorage.getItem('dexters_pc_pos_sales_v3')||'[]').some(x=>x.kind==='money_owed'&&x.method==='card'&&x.mode==='LIVE'),null,{timeout:7000});const debt=await page.evaluate(()=>JSON.parse(localStorage.getItem('dexters_pc_pos_sales_v3')||'[]'));if(!debt.some(x=>x.kind==='money_owed'&&x.method==='card'&&x.mode==='LIVE'))fail('Money Owed live payment missing',debt.slice(0,5));console.log('PASS MONEY OWED LIVE PAYMENT');

 await closeAll(page);await clickId(page,'pcSundayBtn');await page.waitForSelector('#srLiveModal:not(.srHide)');await page.waitForSelector('#srCustomerQuery');await page.waitForSelector('.srSlotBtn');const slotCount=await page.locator('.srSlotBtn').count();if(slotCount<1)fail('Sunday Roast touchscreen collection slots missing',{slotCount});await page.locator('#srCustomerQuery').fill('Browser');await page.locator('#srCustomerGo').click();await page.waitForSelector('.srCustomerPick');await page.locator('.srCustomerPick').first().click();const roastText=await page.locator('#srLiveModal').innerText();for(const t of ['Roast Chicken Dinner','Roast Beef Dinner','Adult Mixed Roast Dinner',"Kids' Roast Chicken Dinner","Kids' Roast Beef Dinner",'Kids Mixed Roast Dinner','Extra Yorkshire Pudding','Extra Gravy','Extra Stuffing','Extra Roast Potatoes','Extra Chicken','Extra Beef'])if(!roastText.includes(t))fail('Sunday Roast option missing '+t);await page.locator('[data-sr-plus="meals:mixed"]').click();await page.locator('[data-sr-plus="extras:gravy"]').click();if((await page.locator('#srTotal').textContent())!=='£15.99')fail('Sunday Roast total wrong',await page.locator('#srTotal').textContent());await page.evaluate(()=>document.getElementById('srLiveModal')?.classList.add('srHide'));console.log('PASS SUNDAY ROAST FULL OPTIONS + EXTRAS');
