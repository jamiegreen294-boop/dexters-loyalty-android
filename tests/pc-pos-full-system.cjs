...
(async()=>{
 ...
 await closeAll(page);const popupPromise=page.waitForEvent('popup',{timeout:3000}).catch(()=>null);await clickId(page,'pcSundayBtn');const roastPage=await popupPromise;if(roastPage){await roastPage.waitForLoadState('domcontentloaded').catch(()=>{});if(!roastPage.url().includes('pos-sunday-roast-tab.html'))fail('Sunday Roast did not open its dedicated tab',roastPage.url());await roastPage.close().catch(()=>{})}console.log('PASS SUNDAY ROAST DEDICATED TAB');
