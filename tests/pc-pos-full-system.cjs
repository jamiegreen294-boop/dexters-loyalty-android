...
(async()=>{
 ...
 await clickId(page,'reloadBtn');await page.waitForFunction(()=>Array.isArray(S.cats)&&S.cats.length>=8);await clickId(page,'updateBtn');if(!(await page.locator('#status').textContent()).includes('update'))fail('Update button did not run');console.log('PASS CORE ORDER TOOL BUTTONS');
