...
window.SundayApi={sb,call,config,status:async date=>{const d=await call({action:'status',collection_date:date});return{...d,config:config(d.settings)}},create:(date,d,requestId)=>call({action:'create',collection_date:date,collection_slot:d.slot,customer_name:d.name,customer_phone:d.phone,meals:d.meals,extras:d.extras,request_id:requestId}),myOrders:()=>call({action:'my_orders'}),settings:date=>call({action:'settings',collection_date:date}),setSettings:(date,c)=>call({action:'set_settings',collection_date:date,enabled:c.enabled,cutoff_mode:c.override,slots:c.slots,stock:c.stock}),prep:date=>call({action:'prep_summary',collection_date:date}),allOrders:date=>call({action:'all_orders',collection_date:date}),kds:date=>call({action:'kds_orders',collection_date:date}),transition:(id,action,reason)=>call({action,id,reason})};
})();
