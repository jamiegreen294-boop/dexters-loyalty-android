...
const pin=fs.readFileSync(path.join(out,'pos-pc-pin-login.js'),'utf8');for(const required of ['SIGN IN WITH PIN','ACTIVATE PIN LOGIN','6-digit setup code','pc-pos-pin-auth','device_secret','dexters-pos-session'])if(!pin.includes(required))throw new Error('PIN login layer missing '+required);
...
console.log('PASS GitHub Pages PC POS build: secure PIN-first sign-in, scanner pairing, core POS, offline retry/dedupe, scheduled promotions, recipe ingredient deduction, supplier purchasing/receiving, expiry/stocktake, Money Owed reminders/statements, kitchen busy mode/capacity, close-day workflow, customer phone, loyalty, phone, WhatsApp, customer 360, split/merge, ingredient stock, digital receipts/invoices, customer display, manager and security audit controls present');
