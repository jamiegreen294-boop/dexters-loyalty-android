...
const roast=fs.readFileSync(path.join(out,'pos-sunday-roast.js'),'utf8');for(const required of ['pos-sunday-roast-tab.html','DextersSundayOrders']) if(!roast.includes(required)) throw new Error('Sunday Roast launcher missing '+required);const roastTab=fs.readFileSync(path.join(out,'pos-sunday-roast-tab.html'),'utf8');for(const required of ['SUNDAY ROAST','Outstanding balance due before collection','SQUARE CARD','status_change','pc-pos-square-bridge','Walk-in / non-loyalty']) if(!roastTab.includes(required)) throw new Error('Sunday Roast tab missing '+required);
