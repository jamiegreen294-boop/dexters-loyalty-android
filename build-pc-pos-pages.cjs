...
for(const f of ['pos-pc-v3-addon.js','pos-pc-scanner.js','pos-pc-loyalty-test.js','pos-pc-category-home.js','pos-order-management.js','pos-pc-manager.js','pos-pc-table-payments.js']){
...
for(const required of ['pos-pc-v3-addon.js','pos-pc-scanner.js','pos-pc-loyalty-test.js','pos-pc-category-home.js','pos-order-management.js','pos-pc-manager.js','pos-pc-table-payments.js','dexters-table-order-pc-test','uber-table-service-pc-test-api']){
  if(!check.includes(required)) throw new Error('GitHub Pages PC POS missing '+required);
}
for(const bad of ['src="/pos-pc-','src="/pos-order-management.js"']) if(check.includes(bad)) throw new Error('Root-relative PC POS asset path leaked into GitHub Pages build: '+bad);
...
console.log('PASS GitHub Pages PC POS build: category cards, scanner, loyalty, held/table/QR accounts, Square, refunds, drawer, manager, part-pay/split-bill and safe table routes present');
