...
if(!fs.existsSync('dist/pos-sunday-roast-tab.html'))throw new Error('Missing pos-sunday-roast-tab.html');
fs.copyFileSync('dist/pos-sunday-roast-tab.html',path.join(out,'pos-sunday-roast-tab.html'));
...
fs.writeFileSync(path.join(out,'sw.js'),`const C='dexters-pos-pc-v3-20260920-loyalty-receipt-print-v14';self.addEventListener('install',e=>{self.skipWaiting()});self.addEventListener('activate',e=>e.waitUntil((async()=>{for(const k of await caches.keys())if(k!==C)await caches.delete(k);await self.clients.claim();const cs=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const c of cs){try{c.postMessage({type:'DEXTERS_POS_CACHE_RESET',build:C})}catch{}}})()));self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;const req=new Request(e.request,{cache:'no-store'});e.respondWith(fetch(req).catch(()=>caches.match(e.request)))});`);
...
const scannerPage=fs.readFileSync(path.join(out,'scanner','index.html'),'utf8');for(const required of ["BACKUP LOYALTY SCANNER",'claim_pair','send_scan','START CAMERA SCANNER'])if(!scannerPage.includes(required))throw new Error('Foodhub scanner page missing '+required);
...
const roast=fs.readFileSync(path.join(out,'pos-sunday-roast.js'),'utf8');for(const required of ["This week's orders",'CREATE LIVE SUNDAY ROAST ORDER','Walk-in customer','customer_search','TAKE PAYMENT','pc-pos-square-bridge','DextersSundayOrders']) if(!roast.includes(required)) throw new Error('Sunday Roast full screen missing '+required);const roastTab=fs.readFileSync(path.join(out,'pos-sunday-roast-tab.html'),'utf8');for(const required of ['__DEXTERS_SUNDAY_STANDALONE','pos-sunday-roast.js','Back to POS']) if(!roastTab.includes(required)) throw new Error('Sunday Roast tab shell missing '+required);
