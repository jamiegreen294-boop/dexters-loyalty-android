...
html:not([data-dexters-theme-mirror-ready="1"]) body{visibility:hidden}html[data-dexters-theme-mirror-ready="1"] body{visibility:visible}
</style>`;
const js=`<script id="dextersWebsiteThemeMirrorBootstrapTest">(function(){function ready(){document.documentElement.setAttribute('data-dexters-theme-mirror-ready','1')}try{var body=document.body;if(!body){ready();return}if(document.documentElement.dataset.dextersThemeReady==='1'){ready();return}var done=false;var mo=new MutationObserver(function(){if(done)return;if(document.documentElement.dataset.dextersThemeReady==='1'||/season-/.test(body.className)){done=true;mo.disconnect();ready()}});mo.observe(body,{attributes:true,attributeFilter:['class']});setTimeout(function(){if(!done){done=true;mo.disconnect();ready()}},700)}catch(e){ready()}})();</script>`;
...
fs.writeFileSync(path,html);console.log('Website theme mirror TEST layer applied');
