...
function start(){
 ...
 const dexter=node('section','work-dexter-chat');dexter.id='workDexterChat';dexter.hidden=true;dexter.innerHTML='<div class="work-dexter-head"><div><div class="work-heading" style="margin:0">Ask Dexter</div><div class="work-dexter-sub">Dexter’s AI, right here in the app.</div></div><button type="button" id="workDexterClose" class="work-dexter-close" aria-label="Close Ask Dexter">×</button></div><div id="workDexterMessages" class="work-dexter-messages" aria-live="polite"></div><form id="workDexterForm" class="work-dexter-form"><textarea id="workDexterInput" rows="2" maxlength="2000" placeholder="Message Dexter…" aria-label="Message Dexter"></textarea><button type="submit" id="workDexterSend">Send</button></form><div id="workDexterStatus" class="work-dexter-status" role="status"></div>';
 let dexterHistory=[];
 ...
 $('workRewards').onclick=()=>nav('qrPage');$('workNavRewards').onclick=()=>nav('qrPage');$('workNavAccount').onclick=()=>nav('accountPage');$('workSpin').onclick=()=>nav('spinPage');$('workDexter').onclick=openDexter;$('workNavDexter').onclick=openDexter;$('workDexterClose').onclick=()=>{dexter.hidden=true;syncWorkNav()};$('workDexterForm').addEventListener('submit',e=>{e.preventDefault();const input=$('workDexterInput'),text=input.value.trim();if(!text)return;input.value='';sendDexter(text)});
