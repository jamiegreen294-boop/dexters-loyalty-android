...
function start(){
 ...
 const homeNav=node('nav','');homeNav.id='workHomeNav';homeNav.setAttribute('aria-label','Customer home navigation');homeNav.innerHTML='<button type="button" id="workNavHome" class="active" aria-current="page"><b>🏠</b>Home</button><button type="button" id="workNavRewards"><b>🎁</b>Rewards</button><a href="/collection-order-test.html"><b>🍔</b>Order</a><button type="button" id="workNavLive"><b>🧾</b>Live Order</button><button type="button" id="workNavAccount"><b>👤</b>Account</button>';document.body.append(homeNav);
 ...
 $('workRewards').onclick=()=>nav('qrPage');$('workNavRewards').onclick=()=>nav('qrPage');$('workNavAccount').onclick=()=>nav('accountPage');$('workNavLive').onclick=()=>{nav('homePage');setTimeout(()=>document.getElementById('workCurrentBody')?.scrollIntoView({behavior:'smooth',block:'center'}),80)};$('workSpin').onclick=()=>nav('spinPage');
