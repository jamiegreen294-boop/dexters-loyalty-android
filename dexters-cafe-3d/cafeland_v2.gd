...
var game_hour := 8.0
var served := 0
var shift_open := false
var rush := false
var rush_timer := 0.0
var spawn_timer := 0.0
var order_counter := 0
var selected_station := "grill"
var mission_target := 8
var mission_claimed := false
...
var recipes := [
    {"name":"Breakfast Roll","price":4.50,"station":"grill","prep":6.0,"servings":4},
    {"name":"Barista Coffee","price":3.20,"station":"coffee","prep":4.0,"servings":5},
    {"name":"Nero Smash","price":10.50,"station":"grill","prep":8.0,"servings":3},
    {"name":"Loaded Fries","price":6.50,"station":"fryer","prep":7.0,"servings":4},
    {"name":"Street Sub","price":7.50,"station":"prep","prep":5.0,"servings":4},
    {"name":"Chicken Rice Bowl","price":9.50,"station":"prep","prep":7.0,"servings":3}
]
...
func _setup_stations():
    stations = {
        "grill":{"name":"GRILL","pos":Vector3(0.8,0,-3.5),"stock":3,"max_stock":10,"cooking":false,"timer":0.0,"duration":7.0,"level":1,"node":null,"label":null},
        "prep":{"name":"PREP","pos":Vector3(2.5,0,-3.5),"stock":3,"max_stock":10,"cooking":false,"timer":0.0,"duration":6.0,"level":1,"node":null,"label":null},
        "fryer":{"name":"FRYER","pos":Vector3(4.2,0,-3.5),"stock":3,"max_stock":10,"cooking":false,"timer":0.0,"duration":7.0,"level":1,"node":null,"label":null},
        "coffee":{"name":"COFFEE","pos":Vector3(-4.0,0,-3.4),"stock":4,"max_stock":12,"cooking":false,"timer":0.0,"duration":4.0,"level":1,"node":null,"label":null}
    }
...
func _world_tap(screen_pos:Vector2):
    if camera == null:
        return
...
func _build_floor():
    for z in range(-5,8):
        for x in range(-6,7):
            var offset := 0.46 if z % 2 != 0 else 0.0
            var px := float(x)+offset
            if abs(px)>6.7:
                continue
            var shade := 0.0 if (x+z)%3 else -0.06
            var c := C_WOOD.darkened(abs(shade)) if shade < 0 else C_WOOD
            _box("Floor",Vector3(px,0.025,float(z)*0.78+1.0),Vector3(0.94,0.05,0.72),c)

func _build_front_counter():
...
func _build_kitchen():
    ...
    for key in ["grill","prep","fryer"]:
        var s = stations[key]
        var n = _box(s["name"],s["pos"]+Vector3(0,1.38,-0.35),Vector3(1.25,0.36,0.86),C_DARK)
        stations[key]["node"] = n
        var l = Label3D.new()
        l.text = "%s  %d" % [s["name"],s["stock"]]
        l.font_size = 25
        l.modulate = Color.WHITE
        l.position = s["pos"]+Vector3(0,1.92,0.15)
        add_child(l)
        stations[key]["label"] = l
    var cs = stations["coffee"]
    var cl = Label3D.new()
    cl.text = "COFFEE  %d" % cs["stock"]
    cl.font_size = 23
    cl.modulate = C_DARK
...
func _build_dining():
    ...
    for i in range(coords.size()):
        ...
        tables.append({"pos":p,"state":"clean","customer":null,"clean_timer":0.0,"marker":null})
...
func _make_person(label:String,pos:Vector3,shirt:Color,is_player:bool):
    var root = Node3D.new()
    root.name = label
    root.position = pos
    add_child(root)
...
func _spawn_customer():
    ...
    var colors = [Color("#6d9f6a"),Color("#7564ad"),Color("#c96f55"),Color("#4f91a2"),Color("#c38e3f")]
    var c = _make_person("Customer%d"%order_counter,Vector3(-5.8,0,6.5),colors.pick_random(),false)
    c["id"] = order_counter
    c["recipe"] = recipe
    c["state"] = "queue"
    c["patience"] = 100.0
...
func _update_customers(delta):
    for i in range(customers.size()-1,-1,-1):
        var c = customers[i]
        c["patience"] -= delta * (1.7 if rush else 1.0)
        if c["patience"] <= 0.0 and c["state"] not in ["leaving","eating"]:
        ...
        match c["state"]:
            "queue":
                ...
                if c["queue_index"] == 0 and c["node"].position.distance_to(queue_points[0]) < 0.22:
                    c["order_delay"] -= delta
                    if c["order_delay"] <= 0.0:
                        c["state"] = "waiting_food"
                        _shift_queue()
            "waiting_food":
                _move_person(c,Vector3(-3.8,0,-1.0),delta)
                var station_key:String = c["recipe"]["station"]
            ...
            "pickup":
                ...
                if c["node"].position.distance_to(Vector3(5.25,0,-1.1)) < 0.3:
                    c["pickup_delay"] -= delta
                    if c["pickup_delay"] <= 0.0:
                        if c["sit_in"]:
                            var table_idx := _find_clean_table()
                            if table_idx >= 0:
                                c["table"] = table_idx
                                tables[table_idx]["state"] = "occupied"
                                tables[table_idx]["customer"] = c
                                c["state"] = "eating"
                        ...
                        if not c["sit_in"]:
                            ...
                            c["state"] = "leaving"
            "eating":
                var t = tables[int(c["table"])]
                _move_person(c,t["pos"]+Vector3(0,0,0.45),delta)
                c["eat_timer"] -= delta
                if c["eat_timer"] <= 0.0:
                    _complete_sale(c)
                    var ti := int(c["table"])
                    tables[ti]["state"] = "dirty"
                    tables[ti]["customer"] = null
                    c["state"] = "leaving"
            "leaving":
...
func _update_stations(delta):
    for key in stations.keys():
        ...
        if s["cooking"]:
            ...
            if s["timer"] <= 0.0:
                s["cooking"] = false
                var add_amount := 4 + int(s["level"])
                s["stock"] = min(int(s["max_stock"]),int(s["stock"])+add_amount)
                _toast("%s ready: +%d servings" % [s["name"],add_amount])
        stations[key] = s
        if s["label"] != null:
            var suffix := "  %.0fs" % max(0.0,float(s["timer"])) if s["cooking"] else "  %d" % int(s["stock"])
            s["label"].text = s["name"] + suffix

func _start_batch(station_key:String):
    selected_station = station_key
    var s = stations[station_key]
    if s["cooking"]:
...
func _update_staff(delta):
    ...
    if cleaning_idx >= 0 and cleaner["node"].position.distance_to(clean_target) < 0.25:
        tables[cleaning_idx]["clean_timer"] += delta
...
func _update_player(delta):
    ...
    for key in stations.keys():
        if player["node"].position.distance_to(stations[key]["pos"]+Vector3(0,0,1.0)) < 1.4 and stations[key]["cooking"]:
...
func _complete_sale(c):
    ...
    var tip := max(0.0,(float(c["patience"])-35.0)/100.0)*1.5
    if rush:
        tip += 0.5
    cash += base+tip
    served += 1
...
func _build_ui():
    ...
    ui["toast_panel"] = Panel.new()
    ui["toast_panel"].anchor_left = 0.12
    ui["toast_panel"].anchor_right = 0.88
    ui["toast_panel"].anchor_top = 0.69
    ui["toast_panel"].anchor_bottom = 0.75
    ui["toast_panel"].add_theme_stylebox_override("panel",_style(Color("#22201fd8"),18))
    layer.add_child(ui["toast_panel"])
    ui["toast"] = _label(ui["toast_panel"],Vector2(10,6),Vector2(520,48),"Welcome",17,Color.WHITE)

    ui["open"] = Button.new()
    ui["open"].anchor_left = 0.35
    ui["open"].anchor_right = 0.65
    ui["open"].anchor_top = 0.87
    ui["open"].anchor_bottom = 0.93
    ui["open"].text = "OPEN"
    ui["open"].add_theme_font_size_override("font_size",25)
    ui["open"].add_theme_color_override("font_color",Color.WHITE)
    ui["open"].add_theme_stylebox_override("normal",_style(C_RED,20))
    ui["open"].add_theme_stylebox_override("pressed",_style(C_RED.darkened(0.1),20))
    ui["open"].pressed.connect(_toggle_shift)
    layer.add_child(ui["open"])

func _station_button(parent:Control,text:String,key:String):
    var b = Button.new()
    b.text = text
...
func _small_button(parent:Control,text:String,call:Callable,col:Color):
    var b = Button.new()
    b.text = text
...
func _upgrade_selected_station():
    var s = stations[selected_station]
    var cost := 100 + int(s["level"])*60
    if cash < float(cost):
...
func _label(parent:Control,pos:Vector2,size:Vector2,text:String,font_size:int,col:Color):
    var l = Label.new()
    l.position = pos
    l.size = size
    l.text = text
...
func _progress(parent:Control,pos:Vector2,size:Vector2,value:float,max_value:float,col:Color):
...
func _part_box(parent:Node3D,pos:Vector3,size:Vector3,c:Color):
    var mi = MeshInstance3D.new()
    var m = BoxMesh.new()
    m.size = size
    mi.mesh = m
...
func _part_sphere(parent:Node3D,pos:Vector3,size:Vector3,c:Color):
    var mi = MeshInstance3D.new()
    var m = SphereMesh.new()
    m.radius = 0.5
    m.height = 1.0
...
func _part_capsule(parent:Node3D,pos:Vector3,r:float,h:float,c:Color):
    var mi = MeshInstance3D.new()
...
func _cylinder_child(parent:Node3D,pos:Vector3,r:float,h:float,c:Color):
    var mi = MeshInstance3D.new()
    var m = CylinderMesh.new()
    m.top_radius = r
    m.bottom_radius = r
