(()=>{
  ...
  function pair(){const current=localStorage.getItem(PAIR_KEY)||'';const code=prompt('Enter the 6-digit pairing code shown by the Dexter scanner bridge on this PC:',current);if(code===null)return;const cleaned=String(code).replace(/\D/g,'').slice(0,6);if(cleaned.length!==6){status('Scanner pairing code must be 6 digits');return}localStorage.setItem(PAIR_KEY,cleaned);lastId=0;status('Scanner pairing saved · checking local bridge…');beginPolling();poll()}
  function installButton(){const top=document.querySelector('.top');if(!top||document.getElementById('scannerBtn'))return;const b=document.createElement('button');b.id='scannerBtn';b.textContent='Scanner';b.title='Pair Foodhub scanner';const staff=document.getElementById('staffBtn');top.insertBefore(b,staff||null);b.onclick=pair}
