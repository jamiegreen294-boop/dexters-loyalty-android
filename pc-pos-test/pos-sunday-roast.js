...
async function takePayment(o){
 ...
 try{
   if(typeof window.DextersPcTakePayment!=='function')throw Error('POS payment screen is not ready. Refresh the POS and try again.');
   const paid=await window.DextersPcTakePayment(due,'Sunday Roast · '+title);
   await call({action:'payment',id:o.id,payment_method:paid.method,payment_meta:paid});
   await refresh();
 }catch(e){
   const msg=String(e?.message||e||'Payment not completed');
   if(msg && !/cancel/i.test(msg))alert(msg);
 }
}
