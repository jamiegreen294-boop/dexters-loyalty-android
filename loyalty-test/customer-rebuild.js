(() => {
  ...
  async function refresh(force=false){
    ...
    ensureCards();
    refreshPromise=Promise.allSettled([loadProfileAndCoffee(),loadAppRewards(),loadOrders()])
  ...
  function bind(){
    ...
    window.addEventListener('storage',e=>{if(e.key===SESSION_KEY)setTimeout(()=>refresh(true),0)});
